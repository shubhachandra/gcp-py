...
Sub AllocateCIDRsBasedOnTSize()
    Dim wsUI As Worksheet
    Set wsUI = ThisWorkbook.Sheets("UI")

    Dim tsize As String, sdlc As String, region As String, compute As String
    tsize = Trim(wsUI.Range("D13").Value)
    sdlc = Trim(wsUI.Range("D14").Value)
    region = Trim(wsUI.Range("D15").Value)
    compute = LCase(Trim(wsUI.Range("D16").Value))

    Dim prefixPrimary As String, prefixNonRoutable As String
    Dim prefixPUPI As String, prefixMaster As String

    Select Case tsize
        Case "Micro-XS"
            prefixPrimary = "/28": prefixNonRoutable = "/27": prefixPUPI = "/23": prefixMaster = "/28"
        Case "Small-S"
            prefixPrimary = "/27": prefixNonRoutable = "/26": prefixPUPI = "/22": prefixMaster = "/28"
        Case "Medium"
            prefixPrimary = "/26": prefixNonRoutable = "/25": prefixPUPI = "/21": prefixMaster = "/28"
        Case "Large"
            prefixPrimary = "/25": prefixNonRoutable = "/24": prefixPUPI = "/20": prefixMaster = "/28"
        Case "Xtra-Large"
            prefixPrimary = "/24": prefixNonRoutable = "/23": prefixPUPI = "/19": prefixMaster = "/28"
        Case Else
            MsgBox "Invalid T-size specified in D13.", vbExclamation
            Exit Sub
    End Select

    ' Output all selected rows into UI sheet only
    Call FilterAndWriteToUI("primary_range", prefixPrimary, sdlc, region, 23)
    Call FilterAndWriteToUI("nonroutable_range", prefixNonRoutable, sdlc, region, 25)
    Call FilterAndWriteToUI("PUPI-ranges", prefixPUPI, sdlc, region, 27)

    If compute <> "composer" Then
        Call FilterAndWriteToUI("masterip", prefixMaster, sdlc, region, 29)
    End If
End Sub

Sub FilterAndWriteToUI(sheetName As String, prefixFilter As String, sdlc As String, region As String, uiRow As Long)
    Dim sourceWS As Worksheet, uiWS As Worksheet
    Set sourceWS = ThisWorkbook.Sheets(sheetName)
    Set uiWS = ThisWorkbook.Sheets("UI")
    ...
    Dim i As Long
    For i = 2 To lastRow
        If Trim(sourceWS.Cells(i, 1).Value) = prefixFilter And _
           LCase(Trim(sourceWS.Cells(i, 6).Value)) = LCase(sdlc) And _
           LCase(Trim(sourceWS.Cells(i, 7).Value)) = LCase(region) And _
           LCase(Trim(sourceWS.Cells(i, 3).Value)) = "available" Then
            candidates.Add i
        End If
    Next i

    If candidates.Count = 0 Then
        MsgBox "No available subnet found in '" & sheetName & "' for prefix " & prefixFilter, vbInformation
        Exit Sub
    End If
    ...
    selectedRow = candidates(Int(Rnd() * candidates.Count) + 1)

    ' Copy columns A to H from source to UI sheet
    Dim j As Long
    For j = 1 To 8 ' Columns A to H
        uiWS.Cells(uiRow, j).Value = sourceWS.Cells(selectedRow, j).Value
    Next j
End Sub
